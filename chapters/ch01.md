...
- **cloud-controller-manager:** load balancers, node instances.

The data plane runs it, on every node:
...
flowchart LR
  ...
  SCHED -->|watch pods write bindings| API
  KCM -->|watch reconcile objects| API
  CCM -->|reconcile cloud resources| API
  KUBELET -->|watch pods report status| API
  KPROXY -->|watch services endpointslices| API
  ...
  class API leader
  class ETCD,PODS resource
  class SCHED,KCM,CCM,KUBELET,KPROXY process
...
1. **kubectl** loads kubeconfig for the server address and credentials, then sends the change: a POST create on first apply, a client-side-apply PATCH on later applies (both carry the last-applied-configuration annotation), or a Server-Side Apply patch with `--server-side` (Chapter 2).
   - Before sending, kubectl maps the manifest's kind to a REST path via API discovery, and a GET (404 on first apply) decides create vs patch.
...
- **"kubectl talks to the nodes."** kubectl only talks to the API server; even `logs` and `exec` are proxied through it to the kubelet (Chapter 4 traces the path).
