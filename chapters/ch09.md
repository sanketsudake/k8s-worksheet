...
This chapter closes the standards story. It covers how the kubelet drives containers (CRI), how hardware reaches pods (device plugins, and their successor DRA), and the map of every extension interface. That map is the diagram to reproduce on a whiteboard when asked "how would you extend Kubernetes?". In Flow 8 terms, this chapter owns the CRI segment — sandbox creation through container start (steps 4–7), with Chapter 7 zooming into the CNI call inside it. The mental model: Kubernetes is a kernel with well-defined syscall-like interfaces; everything vendor-specific lives behind one of them.
...
The **PodSandbox** is CRI's key concept: the pod's shared environment, created before any container. In practice it is the `pause` container — a tiny process that holds the pod's network and IPC namespaces so containers can restart without losing the pod's identity or IP. `RunPodSandbox` is also where the runtime execs the CNI plugins (Flow 21).

Implementations: **containerd** (with its CRI plugin) and **CRI-O** are the mainstream runtimes; dockershim was removed in v1.24. Below them sits the OCI layer: the runtime shells out to an OCI runtime to actually create the Linux process.

- **runc:** namespaces + cgroups, the default.
- **gVisor:** `runsc`, a user-space kernel intercepting syscalls.
- **Kata:** a lightweight VM per pod.

A pod selects among them with `runtimeClassName`; RuntimeClass maps to a configured runtime handler.

```mermaid
flowchart LR
    Kubelet -->|CRI gRPC| RT[containerd or CRI-O]
    RT -->|OCI spec| RUNC[runc default]
    RT -->|OCI spec| GV[gVisor kernel]
    ...
    class Kubelet process
    class RT,RUNC,GV,KATA,CNIP external
```
...
Migration is gradual since v1.37: a DeviceClass can carry an extended resource name, so a pod that still asks for `nvidia.com/gpu: 1` is served by the DRA driver with no claim in its spec (GA). Devices can carry taints (also cluster-wide via DeviceTaintRule), and a claim's `.status.devices` reports per-device state such as a NIC's IP (both GA).

## The extension map

One picture of where every standard plugs in: left of each arrow, the core component; right, what you implement or install.
...
flowchart LR
    ...
    Kubelet
    RT[CRI runtime]
    API -->|admission| Hooks[webhooks and CEL]
    API -->|new types| CRDs[CRDs and controllers]
    API -->|delegates| Agg[aggregated servers]
    API -->|encrypts| KMS[KMS plugin]
    Sched -->|hooks| SPlug[scheduler plugins]
    CCM -->|LB and routes| Cloud[cloud provider API]
    Kubelet -->|CRI| RT
    RT -->|exec| CNIx[CNI plugins]
    Kubelet -->|stage publish| CSIx[CSI drivers]
    Kubelet -->|prepare| DRAx[DRA and device plugins]
    classDef process fill:#38bdf8,stroke:#0369a1,color:#fff
    classDef external fill:#64748b,stroke:#334155,color:#fff
    class API,Sched,CCM,Kubelet,RT process
    class Hooks,CRDs,Agg,KMS,SPlug,Cloud,CNIx,CSIx,DRAx external
```
*Figure 9.2 — every vendor integration hangs off one of ten interfaces; know which component owns each arrow.*

The API server extends through admission (webhooks, or in-process CEL policies, GA v1.30), new types (CRDs, or aggregated servers like metrics-server), and a KMS plugin for encryption at rest. The scheduler extends through framework plugins (Chapter 3); the cloud controller manager isolates provider code. On the node the kubelet speaks CRI to the runtime, CSI for volumes, and DRA or device plugins for hardware, while the runtime execs CNI.

Adjacent but niche: NRI (hooks inside containerd/CRI-O) and the kubelet's CPU/Topology/Memory managers — policies, not plug-in points.

## Flows

### Flow 26: What happens when a pod requests a GPU via DRA
...
12. **Kubelet** on pod deletion calls `NodeUnprepareResources`; the claim's reservation is cleared, and a claim from a template is deleted with its pod, freeing the device for reallocation.
...
sequenceDiagram
    ...
    participant Kubelet
    participant CRI
    Driver->>API: publish slices
    User->>API: create pod
    KCM->>API: create claim
    Sched->>Sched: evaluate CEL
    Sched->>API: write allocation
    Sched->>API: bind node
    Kubelet->>Driver: prepare resources
    Driver-->>Kubelet: CDI IDs
    Kubelet->>CRI: create container
    Note over Sched,Driver: no driver call
```
*Figure 9.3 — structured parameters let the scheduler allocate from published inventory; the driver acts only on the node.*

**Where this can fail**

- **Symptom:** pod `Pending`, "no nodes satisfy the claim".
    - **Cause:** CEL selector matches no published device, or all matching devices are allocated.
    - **Where to look:** ResourceSlices (`kubectl get resourceslices`), claim status, other claims' reservations.
- **Symptom:** pod `Pending` and ResourceSlices are empty.
    - **Cause:** driver DaemonSet not running or failing discovery.
    - **Where to look:** driver node pods, their registration logs.
- **Symptom:** pod scheduled but stuck before start.
    - **Cause:** `NodePrepareResources` failing — driver socket unregistered, kernel modules missing.
    - **Where to look:** kubelet log, driver node plugin log.
- **Symptom:** container starts but the app sees no GPU.
    - **Cause:** CDI spec incomplete (device nodes present, libraries missing) or runtime lacks CDI support.
    - **Where to look:** CDI spec files on the node, runtime config.
...
*Strong answers also mention:* sandbox creation is where CNI is invoked (Flow 21); in Kata, the "sandbox" is a VM, showing the abstraction earns its keep.

**Q 9.3 — Name the DRA API objects and their storage analogs.**

**Answer.** The DRA objects and their storage analogs:

- **ResourceClaim** ≈ PVC: a request for a device, with a lifecycle and exclusivity.
- **ResourceClaimTemplate:** stamps a claim per pod.
- **DeviceClass** ≈ StorageClass: admin-curated selection defaults.
- **ResourceSlice:** the driver's published inventory — no storage analog, but it's the piece that lets the scheduler allocate without calling the driver.

GA since v1.34 with structured parameters.
...
*Strong answers also mention:* device plugins remain fine for simple countable devices, and the extended-resource bridge (GA in v1.37) moves such pods to a DRA driver with no spec change.
...
**Answer.** They are OCI-level runtimes, not CRI implementations: containerd/CRI-O still serve CRI, but hand the OCI spec to a different handler. RuntimeClass maps a name to a handler configured in the runtime; the kubelet passes the handler in `RunPodSandbox`.

- **runc:** namespaces+cgroups on the shared kernel.
- **gVisor:** interposes a user-space kernel (syscall filtering, some incompatibility and overhead).
- **Kata:** boots a micro-VM per pod (hardware isolation, more memory, slower start).

Scheduling is unaffected except for RuntimeClass `overhead` accounting and node selection for nodes that have the handler.
...
**Answer.** Choose DRA — partitioning and attribute selection are exactly its gaps over device plugins. Build:

1. A node agent publishing ResourceSlices with per-device and per-partition attributes (memory, generation, links), modeling partitions as allocatable devices.
2. A DeviceClass per common request shape.
3. The node gRPC service implementing `NodePrepareResources`/`NodeUnprepareResources`, creating partitions on demand and emitting CDI specs.
4. Optionally CEL admission validation for claim sanity.

Test with overconstrained claims, and verify autoscaler simulation sees your slices. No scheduler or kubelet changes — that's the point of the interface.

*Strong answers also mention:* CDI as the runtime-facing contract (device nodes, mounts, env) and driver upgrade behavior — slices must be republished consistently to avoid double-allocation — and device compatibility groups (alpha in v1.37), so the scheduler rejects a MIG-plus-vGPU mix up front.
