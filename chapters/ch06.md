...
**Conditions.** Use `metav1.Condition`: `Type` (CamelCase, positive polarity — `Available`, `Ready`), `Status` (True/False/Unknown), `Reason`, `Message`, `LastTransitionTime`, `ObservedGeneration`. Set them with `meta.SetStatusCondition`, which moves `LastTransitionTime` only on a real flip — hand-rolled code that stamps the time every reconcile is a classic hot-loop source (Flow 17). Publish `status.observedGeneration` so clients can tell "controller has seen my change" from "stale status".
...
- **Owns** — a child type; events map to the owner's key via the `controller: true` ownerReference. No `SetControllerReference`, no requeue when the child changes.
...
**Idempotent, stateless reconciles.** Three forces replay your reconcile: dedup collapses N events into one run at an arbitrary time; restarts replay every object (Flow 18); errors requeue. A reconcile must be safe at any time, any number of times, and must not depend on reconciler-struct fields — that state dies with the pod and lies after failover.
...
**Requeue strategy.** Return `err` → per-item exponential backoff (controller-runtime default 5ms doubling to a 1000s cap, plus a global 10 qps / burst 100 bucket). Return `RequeueAfter: d` only for *time-based* recheck of state no watch covers — external systems, certificate expiry. Never requeue "just in case" after a write: the write itself produces the next event. Returning both an error and a result, the error wins.
...
```mermaid
flowchart TD
  A[reconcile starts] --> B{deletionTimestamp set}
  B -->|no| C{finalizer present}
  C -->|no| D[add finalizer update and return]
  C -->|yes| E[normal create update work]
  B -->|yes| F{finalizer present}
  F -->|no| G[nothing owed return]
  F -->|yes| H[delete external resources]
  H --> I{cleanup complete}
  I -->|no| J[return error to retry]
  I -->|yes| K[remove finalizer and update]
```
...
**Leader election.** Replicas run active-passive: the manager holds a coordination.k8s.io Lease (defaults 15s duration, 10s renew deadline, 2s retry). Only the leader runs controllers; on lost leadership the manager *exits* — Flow 19 shows why.
...
### Flow 17: What happens when one reconcile runs, end to end
...
sequenceDiagram
  ...
  participant Queue
  participant Worker
  API-->>Informer: watch event child deployment changed
  Informer->>Informer: update cache store
  Informer->>Queue: enqueue owner key via ownerReference
  Note over Queue: duplicate keys collapse to one
  Queue->>Worker: pop key
  Worker->>Informer: read app and children from cache
  Worker->>Worker: diff desired against actual
  Worker->>API: status update only because it changed
  API-->>Informer: app status event
  Informer->>Informer: predicate drops status only event
  Worker->>Queue: forget key on success
```
...
### Flow 18: What happens when a controller pod restarts

Your operator's pod is rescheduled; it manages 20,000 CRs.

1. **Kubelet** starts the new pod; the manager campaigns for the leader lease (Flow 19) and wins.
...
sequenceDiagram
  ...
  Ctrl->>Ctrl: workers reconcile each key most are no ops
...
### Flow 19: What happens when leader election hands over
...
5. **MgrB** starts leader-scoped runnables: informers list and watch (a Flow 18 cold start), caches sync, every key reconciles once. Failover latency ≈ lease expiry + cache sync + queue drain.
...
sequenceDiagram
  ...
  MgrB->>API: lease stale after fifteen seconds claim it
...
Two compounding bugs and a smell. (1) `LastSyncTime = metav1.Now()` makes status differ on every run, so the update always writes — and that write is an event on the watched App, which enqueues the next reconcile: a self-triggering hot loop. (2) `RequeueAfter: time.Second` polls on top, and successful runs call `Forget`, so per-item backoff never engages — the only throttle left is the global 10 qps bucket, now spent entirely on self-inflicted work. Multiply by every App: pinned CPU, a stream of no-op status PATCHes, etcd and watch-fan-out churn. (3) The smell: `Requeue: true` after `createDeployment` — with `Owns` wired, the create's watch event already triggers the next run; and if the create errored, the error supersedes the result.
...
*Strong answers also mention:* spec and status conflict independently, cutting resourceVersion contention between users and controller.
...
**Answer.** Wire `Watches(&corev1.ConfigMap{}, handler.EnqueueRequestsFromMapFunc(...))` with a predicate narrowed to that one object; the map function lists Tenants *from the cache* and returns all 10,000 keys. The flood is fine: dedup collapses repeats, and the rate limiter drains at a controlled pace — size `MaxConcurrentReconciles` and the limiter for your convergence target (10,000 keys at 50/s ≈ 3–4 minutes). Keep reconciles cheap and idempotent so the replay is mostly no-ops. Scope the ConfigMap cache with a field selector to the single name.
