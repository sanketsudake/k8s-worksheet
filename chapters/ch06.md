...
2. **Ctrl** (cache) starts informers. Each reflector LISTs its full type — the CRs plus every cached child type — in pages, before watching.
   - The list storm: full payloads decoded into memory at once. Startup is the memory high-water mark; scoped caches shrink it, and server-side sharded list/watch targets exactly this (Flow 3, Chapter 10).
