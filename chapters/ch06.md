...
**Answer.** Wire `Watches(&corev1.ConfigMap{}, handler.EnqueueRequestsFromMapFunc(...))` with a predicate narrowed to that one object; the map function lists Tenants *from the cache* and returns all 10,000 keys. The flood is fine: dedup collapses repeats, and the drain rate is workers × per-reconcile latency — size `MaxConcurrentReconciles` for your convergence target (10,000 keys at 50/s ≈ 3–4 minutes). Keep reconciles cheap and idempotent so the replay is mostly no-ops. Scope the ConfigMap cache with a field selector to the single name.
