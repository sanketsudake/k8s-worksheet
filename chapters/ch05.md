...
- **Symptom:** HPA shows `<unknown>` targets and never scales. **Cause:** metrics-server down or the `metrics.k8s.io` APIService unavailable. **Where to look:** `kubectl top pods`, `kubectl get apiservices`.
- **Symptom:** replicas flap between two sizes. **Cause:** total load sits near the boundary between the two sizes' tolerance bands, or load swings faster than the stabilization window. **Where to look:** HPA events and conditions; set `behavior` scale-down policies.
- **Symptom:** HPA errors "missing request for cpu". **Cause:** a container without CPU requests — utilization is undefined. **Where to look:** pod spec resources.
- **Symptom:** pinned at maxReplicas, latency still bad. **Cause:** the bottleneck is not the scaled metric (database, lock contention). **Where to look:** whether utilization stays high after each scale-up.
- **Symptom:** replicas snap back to a fixed value after every sync. **Cause:** a GitOps tool applies `spec.replicas`, fighting the HPA. **Where to look:** remove `replicas` from the applied manifest so ownership stays with the HPA (Flow 2).
