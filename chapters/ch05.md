...
A controller repeats three steps.

1. **Observe:** read desired state (usually `.spec`) and actual state (children, external resources).
2. **Diff:** compute what is missing, extra, or wrong.
3. **Act:** make the smallest safe change, record observations in `.status`, stop.

The next run starts from scratch — no memory of the last one. Two consequences: reconciles must be idempotent (running twice must not create two of anything), and complete (each run must handle any starting state, because the controller never knows what happened while it was not looking).
...
- **DeltaFIFO** — a queue of per-object deltas (Added/Updated/Deleted/Sync), ordered per object; deduplication of bursts happens later, in the workqueue.
...
```mermaid
flowchart TD
  API[apiserver] -->|list watch| Reflector
  Reflector -->|push deltas| DeltaFIFO
  DeltaFIFO -->|pop order| Loop[informer]
  Loop -->|write objects| Indexer[indexer]
  Loop -->|fire events| Handlers[handlers]
  Handlers -->|enqueue key| Queue[workqueue]
  Queue -->|hand key| Worker[worker]
  Worker -->|read state| Indexer
  Worker -->|write changes| API
  Worker -->|requeue backoff| Queue
  Resync[resync] -->|replay sync| DeltaFIFO
  ...
  class API external
  class Reflector,Loop,Handlers process
  class DeltaFIFO,Queue lease
  class Indexer resource
  class Worker leader
  class Resync standby
```
...
The garbage collector (a controller in kube-controller-manager) watches every resource type via metadata-only watches, builds an owner graph, and deletes orphaned dependents. Deletion offers three propagation policies — background (the kubectl and apps/v1 default), foreground, orphan — walked in Flow 17.
...
```mermaid
stateDiagram-v2
  [*] --> Progressing: template change
  Progressing --> Complete: new RS available
  Progressing --> DeadlineExceeded: no progress in time
  DeadlineExceeded --> Progressing: fix or rollback
  Complete --> Progressing: next template change

  classDef process fill:#38bdf8,stroke:#0369a1,color:#fff
  classDef leader fill:#10b981,stroke:#047857,color:#fff
  classDef standby fill:#94a3b8,stroke:#475569,color:#fff
  class Progressing leader
  class Complete standby
  class DeadlineExceeded process
```

*Figure 5.2 — rollout is a state machine on Deployment conditions; DeadlineExceeded is a signal, not an automatic rollback.*

**StatefulSet.** Pods get stable ordinal identities (`web-0`, `web-1`) and per-ordinal PVCs from `volumeClaimTemplates`. Default `OrderedReady` policy: create 0→N−1, each pod Ready before the next; scale-down and rolling updates run in reverse ordinal order. `partition` holds back updates below an ordinal — the built-in canary knob — and `maxUnavailable` (on by default again in v1.37) lets a rolling update touch more than one pod at a time. PVCs survive pod deletion by default; the opt-in `persistentVolumeClaimRetentionPolicy` (GA since v1.32) can remove them on scale-down or deletion.
...
**CronJob.** The CronJob controller materializes a Job per schedule tick — it manages Jobs, never pods. `concurrencyPolicy` decides what happens if the previous run is still going:

- **Allow:** (default).
- **Forbid:** skip; the skip counts as missed.
- **Replace:** kill and restart.

Missed runs are normal — the controller is level-triggered on wall clock and catches up on its next sync — but with no `startingDeadlineSeconds`, more than 100 missed schedules since the last run stops it entirely ("too many missed start time"). Setting a deadline bounds the counting window and is the standard fix; `timeZone` pins the schedule to a zone instead of the controller's clock.
...
sequenceDiagram
  ...
  participant Sched
  participant Kubelet
  User->>API: patch pod template
  API-->>KCM: deployment updated
  Note over KCM: deployment loop
  KCM->>API: create new RS
  KCM->>API: surge new RS
  KCM->>API: drain old RS
  API-->>KCM: RS updated
  Note over KCM: replicaset loop
  KCM->>API: create pods
  Sched->>API: bind pods
  Kubelet->>API: pods ready
  API-->>KCM: replicas rising
  KCM->>API: step again
```
...
- **Symptom:** rollout stuck at 8/10.
    - **Cause:** new pods unschedulable or failing readiness; with `maxUnavailable` spent, no more old pods die.
    - **Where to look:** `kubectl rollout status`, new-RS pod events.
- **Symptom:** `ProgressDeadlineExceeded` after an image typo.
    - **Cause:** ImagePullBackOff on every new pod.
    - **Where to look:** Deployment conditions, pod statuses; fix or `rollout undo`.
- **Symptom:** 13 pods for 10 replicas mid-rollout.
    - **Cause:** expected — surge.
    - **Where to look:** the maxSurge math, both RSs' `.spec.replicas`.
- **Symptom:** rollout cannot start despite `maxSurge > 0`.
    - **Cause:** ResourceQuota exhausted — surge pods need headroom.
    - **Where to look:** quota events.
- **Symptom:** rollback "loses" a change.
    - **Cause:** `undo` restores the template from the retained RS, not etcd history; with `revisionHistoryLimit: 0` there is nothing to restore.
    - **Where to look:** `kubectl rollout history`.

### Flow 16: What happens when the HPA scales a Deployment

A CPU-based HPA (min 2, max 10, target 60%) watches a Deployment whose pods reach 90% of their requested CPU.

1. **Kubelet** on every node exposes container CPU and memory usage through its summary API (fed by cAdvisor).
2. **Metrics** — metrics-server — scrapes every kubelet, keeps a rolling in-memory snapshot (no history), and serves it as the aggregated `metrics.k8s.io` API (v1 since v1.37; the HPA controller still reads v1beta1).
3. **KCM** (HPA controller) wakes every 15s and reads the target's *scale subresource*: current replicas plus the pod selector.
4. **KCM** queries pod metrics for that selector and averages utilization against per-pod *requests* — here 90% vs the 60% target.
   - Utilization is usage ÷ request; containers without CPU requests make the percentage undefined and the HPA reports an error instead of scaling.
5. **KCM** computes `desired = ceil(current × 90/60)`. A ratio within 10% of 1 (the tolerance) produces no change at all.
6. **KCM** filters the recommendation through `behavior` policies and stabilization: scale-up acts immediately (0s window); scale-down uses the highest recommendation of the last 300s — flap damping.
7. **KCM** writes the new replica count to the scale subresource — its only output. It never touches pods or the pod template.
8. **KCM** (Deployment controller, a separate loop) sees `.spec.replicas` change and scales the ReplicaSet — a pure scale, no new RS, no rollout (Flow 15).
9. **Sched** and **Kubelet** take the new pods to Ready (Flow 8); the EndpointSlice controller adds them and traffic spreads.
   - Unready pods and missing metrics are counted conservatively — 0% usage when scaling up, 100% when scaling down — so half-started pods don't trigger another round.
10. **KCM** (HPA controller) on later ticks sees per-pod utilization diluted toward target; the ratio enters tolerance and the loop settles.
...
sequenceDiagram
  ...
  participant KCM
  participant API
  Metrics->>Kubelet: scrape summary API
  Kubelet-->>Metrics: pod usage
  KCM->>API: read scale subresource
  KCM->>Metrics: query pod metrics
  Metrics-->>KCM: average utilization
  KCM->>KCM: compute desired replicas
  KCM->>KCM: apply stabilization window
  KCM->>API: write scale replicas
  Note over KCM,API: Flow 15 scales the RS
  Note over KCM: next tick ratio in tolerance
```
*Figure 5.4 — the HPA never touches pods: it reads metrics, writes one number to the scale subresource, and Flow 15 does the rest.*

**Where this can fail**

- **Symptom:** HPA shows `<unknown>` targets and never scales.
    - **Cause:** metrics-server down or the `metrics.k8s.io` APIService unavailable.
    - **Where to look:** `kubectl top pods`, `kubectl get apiservices`.
- **Symptom:** replicas flap between two sizes.
    - **Cause:** total load sits near the boundary between the two sizes' tolerance bands, or load swings faster than the stabilization window.
    - **Where to look:** HPA events and conditions; set `behavior` scale-down policies.
- **Symptom:** HPA errors "missing request for cpu".
    - **Cause:** a container without CPU requests — utilization is undefined.
    - **Where to look:** pod spec resources.
- **Symptom:** pinned at maxReplicas, latency still bad.
    - **Cause:** the bottleneck is not the scaled metric (database, lock contention).
    - **Where to look:** whether utilization stays high after each scale-up.
- **Symptom:** `minReplicas: 0` is accepted but the workload never parks at zero.
    - **Cause:** scale to zero (beta, on by default since v1.37) works only with object or external metrics; CPU and memory need running pods.
    - **Where to look:** the HPA's `ScaledToZero` condition, which separates "the HPA parked it" from "someone set replicas to 0".
- **Symptom:** replicas snap back to a fixed value after every sync.
    - **Cause:** a GitOps tool applies `spec.replicas`, fighting the HPA.
    - **Where to look:** remove `replicas` from the applied manifest so ownership stays with the HPA (Flow 2).

### Flow 17: What happens when you delete an object that owns others
...
*Figure 5.5 — propagation policy decides only ordering and whether children die; the GC does the work in every case.*

**Where this can fail**

- **Symptom:** object stuck `Terminating`.
    - **Cause:** a finalizer nobody removes — its controller is down, or foreground deletion blocked by an undeletable dependent.
    - **Where to look:** `metadata.finalizers`, then the controller owning each finalizer.
- **Symptom:** GC stops deleting cluster-wide.
    - **Cause:** GC needs discovery of all types; a broken aggregated APIService stalls its graph.
    - **Where to look:** `kubectl get apiservices` for `False` conditions, KCM logs.
- **Symptom:** pods survive a deployment deletion.
    - **Cause:** orphan propagation, or ownerReferences stripped by a tool.
    - **Where to look:** pod `ownerReferences`, audit log for the DELETE's policy.
- **Symptom:** dependent never collected across namespaces.
    - **Cause:** cross-namespace ownerReferences are invalid.
    - **Where to look:** events mentioning `OwnerRefInvalidNamespace`.
...
**Answer.** An ownerReference names an object's owner; the garbage collector builds a graph from them and deletes dependents when owners go. One reference may be `controller: true`, marking the managing controller — used for adoption and for mapping child events to owners.

- *Background* (default) deletes the owner immediately, children asynchronously after.
- *Foreground* holds the owner Terminating via the `foregroundDeletion` finalizer until `blockOwnerDeletion` dependents are gone.
- *Orphan* strips ownerReferences so children survive.
...
**Q 5.7 — Your HPA flaps between 4 and 8 replicas every few minutes. Why, and which knobs fix it?**

**Answer.** Flapping means the two sizes bracket the load: at 4 replicas, average utilization sits above target, so `ceil(4 × ratio)` says 8; at 8 it falls far enough below target that the recommendation says 4 — and neither lands inside the 10% tolerance. Scale-up applies instantly (0s window) while scale-down waits out the 300s stabilization window, so the pattern is usually: burst up, five quiet minutes, drop, repeat. Fixes, in order:

1. give `behavior.scaleDown` a longer window and a rate policy (say, one pod per minute).
2. widen the per-HPA `tolerance` in `behavior` (GA in v1.37).
3. scale on a steadier signal (requests per second via custom metrics instead of spiky CPU).
4. raise minReplicas above the flap floor.

Never "fix" it by pinning `replicas` in a manifest — that adds a third writer fighting both (Flow 16).

*Strong answers also mention:* the up-fast, down-slow asymmetry is deliberate, and unready-pod conservatism during scale-up already damps one direction.

### Tier 3 — Design & Debug

**Q 5.8 — A rollout is stuck at 8/10 for 30 minutes. Walk me through debugging it.**

**Answer.** Establish which side is stuck: `kubectl get rs` — new RS not gaining available pods, or old not shrinking? Usually the new side: the controller spent its `maxUnavailable` budget and waits on new pods. Check them:

- **Pending:** scheduling (resources, taints, affinity).
- **ImagePullBackOff:** registry/tag.
- **Running-not-Ready:** readiness probe and logs.
- **CrashLoopBackOff:** app or config.

Check ResourceQuota — surge pods need headroom. After 600s the Deployment shows `ProgressDeadlineExceeded`, but nothing auto-rolls-back: decide fix-forward or `rollout undo`.
...
**Q 5.9 — A CR you deleted an hour ago still shows Terminating, and cluster-wide garbage collection seems stopped. Diagnose.**
...
**Q 5.10 — Your CR provisions a namespaced Deployment plus an external DNS record. Design the cleanup: owner references, finalizers, or both?**

**Answer.** Both, split by reach. The Deployment shares the CR's namespace: set a controller ownerReference and GC deletes it for free, even with your controller down. The DNS record has no Kubernetes object, so no ownerReference can cover it: put a finalizer on the CR; on `deletionTimestamp`, delete the record, then remove the finalizer. Order matters — persist the finalizer *before* creating the record, or a crash between the two leaks it (Figure 6.2, next chapter, draws this as the finalizer state machine).

*Strong answers also mention:* ownerReferences cannot cross namespaces or point from a namespaced owner to a cluster-scoped dependent — for those shapes finalizers are the only correct tool.
