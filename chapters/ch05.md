...
**CronJob.** The CronJob controller materializes a Job per schedule tick — it manages Jobs, never pods. `concurrencyPolicy` decides what happens if the previous run is still going: Allow (default), Forbid (skip; the skip counts as missed), Replace (kill and restart). Missed runs are normal — the controller is level-triggered on wall clock and catches up on its next sync — but with no `startingDeadlineSeconds`, more than 100 missed schedules since the last run stops it entirely ("too many missed start time"). Setting a deadline bounds the counting window and is the standard fix; `timeZone` pins the schedule to a zone instead of the controller's clock.
...
**Answer.** Flapping means the two sizes bracket the load: at 4 replicas, average utilization sits above target, so `ceil(4 × ratio)` says 8; at 8 it falls far enough below target that the recommendation says 4 — and neither lands inside the 10% tolerance. Scale-up applies instantly (0s window) while scale-down waits out the 300s stabilization window, so the pattern is usually: burst up, five quiet minutes, drop, repeat. Fixes, in order: give `behavior.scaleDown` a longer window and a rate policy (say, one pod per minute); widen the tolerance; scale on a steadier signal (requests per second via custom metrics instead of spiky CPU); or raise minReplicas above the flap floor. Never "fix" it by pinning `replicas` in a manifest — that adds a third writer fighting both (Flow 16).
