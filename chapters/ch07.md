...
- **IPVS** — in-kernel L4 balancer with hash-table lookup; better at scale, but it still needs iptables underneath, so it is **deprecated**: a warning since v1.35, off by default by v1.40, removed by v1.43.
- **nftables** — GA since v1.33, incremental updates and faster evaluation, talking to the kernel over netlink instead of shelling out to `nft`; the planned Linux default, which is why kube-proxy now warns when no mode is set.
...
5. **Kernel** picks one backend — random-probability rule (iptables or nftables) or IPVS scheduler — and DNATs to `podIP:targetPort`.
...
- **Symptom:** traffic shifts seconds late on some nodes.
    ...
    - **Where to look:** `sync_proxy_rules_duration_seconds`; consider nftables.
...
- IPVS uses hash tables with O(1) lookup, but keeps iptables underneath — deprecated, gone by v1.43.
- nftables (GA v1.33) brings incremental updates and faster evaluation — the successor and the planned default; pick it on any modern kernel.
...
Timeouts rather than refusals mean packets go to an IP that no longer answers — typical of stale rules or conntrack. A minute-long window points at slow iptables full syncs; mitigate with nftables and graceful termination handling.
