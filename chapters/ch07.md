...
- **nftables** — GA since v1.33, incremental updates and faster evaluation, driven over netlink with no `nft` binary on the sync path; the planned Linux default, which is why kube-proxy now warns when no mode is set.
