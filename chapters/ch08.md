...
flowchart TD
    API[kube-apiserver]
    subgraph CPod[controller plugin pod, runs anywhere]
        Prov[external provisioner]
        Att[external attacher]
        CP[controller service gRPC]
    end
    subgraph NPod[node plugin DaemonSet, every node]
        Reg[node driver registrar]
        NP[node service gRPC]
    end
    Prov -->|watches PVCs| API
    Att -->|watches VolumeAttachments| API
    Prov -->|CreateVolume| CP
    Att -->|ControllerPublishVolume| CP
    CP -->|cloud API calls| SB[storage backend]
    Reg -->|registers socket| Kubelet
    Kubelet -->|NodeStage NodePublish| NP
```
...
### Flow 23: What happens when a PVC is created and a pod uses it
...
13. Teardown reverses exactly: containers stop → `NodeUnpublishVolume` → `NodeUnstageVolume` → attach-detach controller deletes the VolumeAttachment → `ControllerUnpublishVolume` → PVC deletion → reclaim policy (`Delete` triggers `DeleteVolume`; `Retain` keeps the PV as `Released`).
...
sequenceDiagram
    ...
    participant Kubelet
    participant CSI
    User->>API: create PVC then pod
    Sched->>API: pick node, annotate PVC with selected node
    Prov->>CSI: CreateVolume with topology
    Prov->>API: create PV, PV controller binds claim
    Sched->>API: bind pod to node
    Att->>CSI: ControllerPublishVolume attach to node
    Kubelet->>CSI: NodeStageVolume format and mount staging path
    Kubelet->>CSI: NodePublishVolume bind mount into pod dir
    Kubelet->>API: pod Running with volume
    Note over CSI: controller service handles create and attach, node service handles stage and publish
```
...
### Flow 24: What happens when a pod with a volume moves to another node
...
sequenceDiagram
    ...
    participant CSI
    participant Kubelet
    Note over API: node A dead, pod force deleted, new pod on node B
    Kubelet->>API: waiting for volume on node B
    KCM->>API: Multi-Attach error, volume still attached to A
    Note over KCM: waits for safety, force detach after 6 minute timeout
    KCM->>API: delete old VolumeAttachment
    Att->>CSI: ControllerUnpublishVolume from A
    Att->>CSI: ControllerPublishVolume to B
    Kubelet->>CSI: NodeStage then NodePublish
    Kubelet->>API: pod Running on B
```
