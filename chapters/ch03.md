...
**Requests, not usage.** Filtering and scoring use pod *requests* against node *allocatable* — never live metrics. On large clusters the scheduler scores only a sample of feasible nodes (`percentageOfNodesToScore`) to bound latency.
...
```mermaid
flowchart LR
  NEW[new pod event] --> AQ[activeQ]
  AQ --> SC[scheduling cycle filter score reserve permit]
  SC -->|node chosen| BC[binding cycle prebind bind]
  SC -->|no fit| US[unschedulable set]
  US -->|relevant cluster event| BQ[backoffQ]
...
4. **Sched** runs Score plugins on feasible nodes (a sample on very large clusters), normalizes and weights, and picks the winner; ties break randomly.
...
flowchart TD
  ...
  PF --> Q1{victims with lower priority exist}
  Q1 -->|no| PARK[pod parked pending waits for event]
  Q1 -->|yes| PICK[pick node with least disruption]
  PICK --> DEL[delete victims gracefully]
  DEL --> NOM[set nominatedNodeName]
  NOM --> RQ[requeue preemptor]
  RQ --> SCHED[schedules when capacity frees]
```
...
- **Symptom:** DaemonSet pods remain on a NoExecute-tainted node. **Cause:** DaemonSet pods get automatic tolerations for the standard node-condition taints — intended. **Where to look:** the pod's tolerations.
...
- **"Scoring runs over all nodes."** Filtering does; scoring can sample on large clusters — no global optimum guaranteed.
