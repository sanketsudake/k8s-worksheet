...
Cluster events that could change the answer — node added, pod deleted, taint removed — move parked pods back toward activeQ; the scheduler tracks which event types can help which pod, so it doesn't retry blindly. With the `GenericWorkload` gate on (beta in v1.37, off by default) a whole PodGroup is one queue entity.
...
- **Node affinity:** selects nodes by labels (required rules filter, preferred rules score).
...
1. **API server** emits the watch event; the scheduler's informer enqueues the unassigned pod into activeQ (after PreEnqueue gates — plugins that can hold a pod out of the queue until it is ready to schedule), ordered by priority.
...
*Strong answers also mention:* Reserve holds resources during the wait — the cost of the rendezvous — and preferring an existing project over a bespoke plugin. v1.37 ships this in-tree: Workload and PodGroup objects (beta, `GenericWorkload` gate off by default) give gang scheduling and workload-aware preemption, so the Permit design is the portable answer and the in-tree one is the direction.
