...
- **Manifest-based admission config (beta in v1.37):** webhooks and CEL policies loaded from disk — enforced from startup, alive while etcd is down, and immune to API deletion.
...
**The watch cache.** The API server keeps one etcd watch per resource type and an in-memory cache of recent versions. Nearly all client watches and many lists are served from this cache — this is what lets thousands of kubelets and controllers watch pods without melting etcd. The cache holds a bounded history window; clients that fall behind must relist (Flow 3).

Periodic **bookmark** events keep idle watchers' resume points fresh. Initialization is resilient (GA in v1.37): while a cache warms, the API server serves bounded requests and answers the rest with 429 and Retry-After instead of forwarding them to etcd.
...
**ServiceAccounts and tokens.** Every pod authenticates as a ServiceAccount. Its token is *projected*: the kubelet requests it from the TokenRequest API — audience-scoped, bound to the pod, expiring (default 1h), refreshed before expiry. No stored Secret is involved; auto-created Secret tokens ended in v1.24. On each request the API server validates signature, expiry, audience, and that the bound pod still exists, so a leaked token dies with its pod.

**Pod certificates and trust bundles.** GA in v1.37, the X.509 sibling of projected tokens. A `podCertificate` projected volume names a signer; the kubelet generates the private key and creates a PodCertificateRequest, and a signer controller fills in the chain and a refresh time. The kubelet writes key and chain into the volume and re-requests at refresh time; NodeRestriction pins requests to the node's own pods. A `clusterTrustBundle` projected volume delivers the CA bundle, so pods get mTLS identity with proof of possession — a peer sees a signature, never the credential.

**RBAC mechanics.** RBAC is deny-by-default and purely additive: ClusterRoles can *aggregate* (a controller merges labeled roles into one), and escalation is prevented — you cannot grant permissions you do not hold, gated by the `escalate` and `bind` verbs.
...
8. **Ctrl** at scale makes this expensive: a big relist from thousands of clients at once is a "list storm" (Chapter 10), where server-side sharded list/watch (alpha) lets a sharded controller's replicas each ask for only their hash range.
...
**Q 2.8 — How does a pod's ServiceAccount token get issued and validated, and why did projected tokens replace Secret-based ones?**

**Answer.** At pod start the kubelet calls the TokenRequest API for the pod's ServiceAccount; the API server mints a signed JWT bound to that pod, scoped to an audience, with a default one-hour lifetime. The kubelet writes it into a projected volume and refreshes it before expiry; well-behaved clients reread the file. On every request the API server checks signature, expiry, and audience — and that the bound pod and ServiceAccount still exist, so a leaked token dies with its pod. Secret-based tokens failed on every axis: unbounded lifetime, no audience, still valid after the pod was gone, and sitting readable in etcd. Auto-creation of them ended in v1.24.

*Strong answers also mention:* audience scoping stops replay of one service's token against another, and in-cluster clients pick up rotation only if they reread the projected file.

### Tier 3 — Design & Debug

**Q 2.9 — After installing a policy product, every deploy across the cluster intermittently fails or takes 10+ seconds. Walk through your debugging.**
...
**Q 2.10 — Two controllers keep flipping the same field on an object, generating write storms. Diagnose and fix.**
...
**Q 2.11 — Design the API-access strategy for a controller managing 50k custom objects without hurting the control plane.**
