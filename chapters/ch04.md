...
sequenceDiagram
  ...
  Kubelet->>Kubelet: node admission check
  Kubelet->>CSI: attach stage and mount volumes
  CSI-->>Kubelet: volumes ready
  Kubelet->>CRI: run pod sandbox
  CRI->>CNI: add pod network
  CNI-->>CRI: pod IP assigned
  CRI-->>Kubelet: sandbox ready
  Kubelet->>CRI: pull images run init containers
  Kubelet->>CRI: start app containers
  Kubelet->>Kubelet: probes pass
  Kubelet->>API: status Running and Ready
  API-->>KCM: watch event pod ready
  KCM->>API: add endpoint to EndpointSlice
  API-->>KProxy: watch event slice updated
  KProxy->>KProxy: program service rules
```
...
sequenceDiagram
  ...
  API-->>Kubelet: watch event terminating
  API-->>KCM: watch event terminating
  KCM->>API: remove endpoint from slice
  API-->>KProxy: slice updated
  KProxy->>KProxy: reprogram rules with lag
  Kubelet->>CRI: run preStop then SIGTERM
  Note over Kubelet: grace period runs
  Kubelet->>CRI: SIGKILL leftovers
  Kubelet->>CRI: remove sandbox
  Kubelet->>API: confirm force delete
  API-->>User: object gone unless finalizers
```
...
```mermaid
flowchart TD
  C[cordon sets unschedulable true] --> T[unschedulable NoSchedule taint]
  T --> S[scheduler filters node out]
  C --> R[running pods unaffected]
  R --> E[endpoints and traffic unchanged]
...
```mermaid
flowchart TD
  P[probe fails threshold times] --> Q{which probe}
  Q -->|readiness| NR[pod Ready false]
  NR --> EP[endpoint marked not ready]
  EP --> TS[traffic stops container untouched]
  TS --> RC[recovers when probe passes]
  Q -->|liveness| K[kubelet kills container]
  K --> RS[restart with backoff]
  RS --> CL[crashloop if it keeps failing]
  Q -->|startup failing| G[liveness and readiness stay gated]
```
...
```mermaid
flowchart LR
  subgraph KubeletPath[kubelet eviction]
    S[memory available below threshold] --> RANK[rank by usage over requests then priority]
    RANK --> EV[evict whole pod status Evicted]
    EV --> RESCH[controller replaces pod elsewhere]
  end
  subgraph KernelPath[kernel OOM kill]
    L[cgroup hits its limit] --> OOM[kill process in container]
    OOM --> RESTART[kubelet restarts container in place]
  end
```
